...
public class EasyWifiMain extends FragmentActivity implements InfoDialog.NoticeDialogListener, SpeedTest.NoticeDialogListener1
{
    ...
    Handler handler1 = new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            if(msg.what==0x1234){
                ...
                User_Image.setImageBitmap(loginimg);
            }
        }
    };
        ...
        public void handleMessage(Message msg) {
            ...
            for (int i = 0; i < mlist.size(); i++) {
                ...
                if (tmp.mac != null) {      //无网络检测
                    ...
                    SetButton(i, tmp.ssid, point, tmp.encryption, tmp.signal, tmp.downlink);
                }
            }
            // TODO: UI界面的更新等相关操作
            //
    ...
    private int[] DrawerItemName = {R.string.app_home, R.string.app_wifi, R.string.app_settings, R.string.app_about,R.string.login_button};
    private int[] DrawerItemIcon = {R.drawable.home, R.drawable.wifi2, R.drawable.settings, R.drawable.about,R.drawable.login_button};
                ...
                try {
                    ...
                    if (httpResponse.getStatusLine().getStatusCode() == 200) {
                        ...
                        if (islocated == true) {
                            ...
                            System.out.println("This speed is " + tmpwifi.downlink + " ---->" + tmpwifi.ssid);
...
// TODO: handle exception
    ...
    public List<WifiInfo> RequestingWifi() {
        return mlist;
    }
 ...
 */
    ...
    private void SetButton(int i, String SSID, LatLng point, String authtype, int signallevel, int downspeed)
    {
       ...
       // LatLng point = new LatLng(39.969052, 116.362668);
        ...
        extrainfo.putInt("downlink", downspeed);
...
//            int i = getArguments().getInt(ARG_PLANET_NUMBER);
    ...
    public static class WifiInfo   //2015.3.10 Changed by Ljm625 设置一个类当做结构体使用
    {
        public String ssid;
        public String mac;
    ...
    {  //TODO: 需要把设置选项判断的代码加进去
        ...
        public void run() {
            ...
            if (networkInfo.getTypeName().toString().trim() != null) {
    ...
    public class UpdateSpeed extends Thread   //更新速度给服务器
    {
        public void run() {
            ...
            try {
                wifiadmin = new WifiTest(EasyWifiMain.this);
                android.net.wifi.WifiInfo wifi = wifiadmin.checkinfo();
                int start = wifi.getSSID().indexOf('"');
                int stop = wifi.getSSID().length();
                String ssid1 = wifi.getSSID().substring(1, stop - 1);
                System.out.println("ServerInfo------>SSID:" + ssid1);
                System.out.println("ServerInfo------>MAC:" + wifi.getBSSID());
                System.out.println("ServerInfo------>DownLink:" + CurrentWifi.downlink);

                HttpPost request = new HttpPost("http://www.52mzone.com/easywifi/ewifispeed.php");
                //2015.3.10 Changed by Ljm625
                List<NameValuePair> params = new ArrayList<NameValuePair>();
                params.add(new BasicNameValuePair("SSID", ssid1));
                params.add(new BasicNameValuePair("MAC_ID", wifi.getBSSID()));
    ...
    private class DrawerItemClickListener implements ListView.OnItemClickListener {
        ...
        private void selectItem(int i) {
            ...
            {
                int i1 = 0;
                WifiInfo tmp = new WifiInfo();
                System.out.println("Doing findings!");
                while (i1 < mlist.size()) {
                    tmp = mlist.get(i1);
                    if (sr1.BSSID.equals(tmp.mac))
                        i1 = mlist.size() + 1;
                    else i1 = i1 + 1;
                }
                if (i1 == mlist.size() + 1) {
                    int level = tmp.signal;
                    String SSID = tmp.ssid;
        ...
        public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
            ...
            if (i == 2) {
                ...
                if (wifiadmin.checkstate()==3){
                    ...
                    //new AuthLogin().wechatAuth(EasyWifiMain.this, authListener);
                    //new AuthLogin().tencentWbAuth(EasyWifiMain.this, authListener);
                }else{
                    Toast.makeText(EasyWifiMain.this,"Wifi is unenable!",Toast.LENGTH_LONG).show();
                }
            } else {
                getActionBar().setTitle(R.string.app_wifi);
                wifif.setWifiList(mlist);
    ...
    public AuthListener authListener = new AuthListener() {
        ...
        public void onAuthSucess(AuthUserInfo userInfo) {
            ...
            // 授权平台为新浪
                ...
                Log.w("Sina_name", userInfo.getSinaProfileImageUrl());
                Log.w("Sina", userInfo.getSinaScreenname());
                Log.w("Sina", userInfo.getSinaAccessToken());

                User_name=(TextView)findViewById(R.id.User_name);
                User_kitname=(TextView)findViewById(R.id.User_kitname);
                User_name.setText(userInfo.getSinaName());
                User_kitname.setText(userInfo.getSinaScreenname());
                url_me=userInfo.getSinaProfileImageUrl();
                new BitMapUpdater().start();
            }
    ...
    class BitMapUpdater extends Thread   //确认WIFI状态，决定是否进行测速
    {  //TODO: 需要把设置选项判断的代码加进去
        @Override
        public void run() {
            // TODO Auto-generated method stub
            System.out.println("Running!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            loginimg=returnBitMap(url_me);
            Message message=new Message();
            message.what=0x1234;
